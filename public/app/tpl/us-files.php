							...
							<?php
								...
								if(!empty($fldArr['dirview'][$usrDir.$getBrowse]['folders'])) {
									foreach($fldArr['dirview'][$usrDir.$getBrowse]['folders'] as $dir) {
										...
										// Folder icon/name
										echo '<div class="col-8 col-md-6"><i style="font-size: 1.4rem; margin-right:6px;" class="far fa-folder"></i> <a href="?page=us-files&browse=' . ltrim($subLink, '/') . '">' . $dir . '</a></div>';
								...
								if(!empty($fldArr['dirview'][$usrDir.$getBrowse]['files'])) {
									...
									foreach($fldArr['dirview'][$usrDir.$getBrowse]['files'] as $file) {
										...
										$fileIco = '<i style="font-size: 1.4rem; margin-right:6px;" class="' . $fIco . '"></i>';
